...
#Giải thích: Qua quá trình chuyển đổi kiểu dữ liệu:
#khi chuyển từ số thập phân sang số nguyên bằng as.integer(), R sẽ loại bỏ phần thập phân mà không làm tròn.
#Khi tiếp tục chuyển sang kiểu ký tự bằng as.character(), giá trị số được hiểu là dữ liệu số mà chỉ còn là chuỗi ký tự dùng để hiển thị, không thể thực hiện các phép toán số học.
...
z2 <- c(x,y)
z2
#c() ghép các chuỗi trên 1 dòng nhưng không thành 1 chuỗi duy nhất 
...
#a)
date_converted <- as.Date(vecDate, format = "%d:%m:%y")
date_converted
...
days_diff <- today - date_converted
days_diff
...
vec3 <- rep(seq(1, 9, by = 2), each = 2, times = 3)
vec3
...
#giải thích: hàm runif tạo các số ngẫu nhiên theo phân phối đều vơi min và max là khoảng để runif bốc

vec5 <- as.integer(runif(50, min=0,max=20))
vec5
